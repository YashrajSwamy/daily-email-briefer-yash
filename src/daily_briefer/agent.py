...
def run_pipeline(config_override: Optional[Config] = None) -> int:
    ...
    try:
        ...
        path = Path(__file__).resolve().parents[2] / "docs" / "js" / "app.js"

        if path.exists():
            text = path.read_text(encoding="utf-8")

            pattern = re.compile(
                r"function\s+triggerWorkflowDispatch\s*\(\s*\)\s*\{[\s\S]*?\n\}",
                re.MULTILINE,
            )
            ...
            if match:
                new_function = f"""function triggerWorkflowDispatch() {{
    window.open('https://github.com/{config.githubRepo}/actions/workflows/daily-brief.yml', '_blank');
    showToast('Opened GitHub Actions runner page. Click "Run workflow" to execute immediately.', 'info');
                ...
                if match.group(0) != new_function:
                    ...
                    logger.info("Updated app.js with GitHub repository URL.")
        ...
        if not profile:
            logger.error(
                "No profile record found (id=1). "
                "Please initialize the database with schema.sql."
            )
            return 1
        ...
        if not is_active:
            logger.info(
                "DailyBriefer is paused (profile.is_active is False). "
                "Exiting cleanly without sending email."
            )
            return 0

        # Resolve dynamic settings (DB overrides env defaults)
        raw_recipient = (
            profile.get("recipient_email", "").strip()
            or config.recipient_email
        )

        if not raw_recipient:
            logger.error(
                "No recipient email configured in database profile or environment."
            )
            return 1
        ...
        primary_model = (
            profile.get("primary_model", "").strip()
            or config.primary_model
        )
        fallback_model = (
            profile.get("fallback_model", "").strip()
            or config.fallback_model
        )
        search_topic = (
            profile.get("search_topic", "").strip()
            or config.search_topic
        )
        search_depth = (
            profile.get("search_depth", "").strip()
            or config.search_depth
        )
        max_queries = (
            profile.get("max_search_queries")
            or config.max_search_queries
        )

        theme_raw = (
            profile.get("theme", "").strip()
            or config.theme
            or "light"
        )
        ...
        theme = (
            theme_raw.lower()
            if theme_raw.lower() in ("light", "dark")
            else "light"
        )
        ...
        logger.info(
            f"Model Stack: Primary='{primary_model}', "
            f"Fallback='{fallback_model}'"
        )
        logger.info(
            f"Search Config: Topic='{search_topic}', "
            f"Depth='{search_depth}', MaxQueries={max_queries}"
        )

        # 3. Read active events
        active_events = load_active_events(supabase)
        logger.info(
            f"Loaded {len(active_events)} active upcoming event milestone(s)."
        )
        ...
        if not articles:
            logger.warning(
                "No articles retrieved from search queries. "
                "Proceeding with synthesis of available context."
            )
        ...
        logger.info(
            f"=== DailyBriefer v2 Pipeline Completed Successfully "
        ...
        logger.exception(
            f"Fatal error in DailyBriefer execution pipeline: {e}"
        )
        return 1
